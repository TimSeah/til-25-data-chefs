...
# --- Configuration (Matching CNNDQN_training_improvement.py) ---
# Environment specific
MAP_SIZE_X = 16
MAP_SIZE_Y = 16
MAX_STEPS = 100 # Max steps in one round of the game (for normalization in manager)
...
OUTPUT_ACTIONS = 5
DROPOUT_RATE = 0.2 # Dropout is handled by model.eval() during inference

# Agent settings for inference
EPSILON_INFERENCE = 0.01 # Small epsilon for some exploration, or 0 for pure exploitation
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


# --- CNN-DQN Model (Identical to CNNDQN_training_improvement.py) ---
...
# --- RL Manager for "better" CNNDQN model ---
class RLManagerBetterCNN: # Renamed class for clarity
    def __init__(self, model_path="my_wargame_cnn_agent_best_100000.pth"): # Default to the specified model
        self.device = DEVICE
        print(f"RLManagerBetterCNN: Using device: {self.device}")
        ...
        if model_path and os.path.exists(model_path):
            try:
                print(f"RLManagerBetterCNN: Loading model state_dict from: {model_path}")
                # This model file contains only the state_dict
                self.model.load_state_dict(torch.load(model_path, map_location=self.device))
                print(f"RLManagerBetterCNN: Loaded model from {model_path}")
            except Exception as e:
                print(f"RLManagerBetterCNN: Error loading model from {model_path}: {e}. Using random weights.")
                self.model.apply(self._initialize_weights)
        else:
            if model_path:
                 print(f"RLManagerBetterCNN: Model path {model_path} not found. Initializing model with random weights.")
            else:
                print("RLManagerBetterCNN: No model path provided. Initializing model with random weights.")
            self.model.apply(self._initialize_weights)
    ...
    def process_observation(self, observation_dict):
        ...
        norm_step = step / MAX_STEPS if MAX_STEPS > 0 else 0.0 # Use MAX_STEPS for manager
        ...
        if state_other_tensor.shape[1] != OTHER_FEATURES_SIZE:
             print(f"RLManagerBetterCNN: Warning: Other features size mismatch. Expected {OTHER_FEATURES_SIZE}, got {state_other_tensor.shape[1]}")
...
if __name__ == '__main__':
    print(f"RL Manager Better CNN - Script Execution Time (UTC): {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime())}")
    print(f"Current User: lolkabash") # From your provided context
    
    # This manager is specifically for "my_wargame_cnn_agent_best_100000.pth"
    # which contains only the state_dict.
    TEST_MODEL_PATH = "my_wargame_cnn_agent_best_100000.pth" 

    if not os.path.exists(TEST_MODEL_PATH):
        print(f"\nWARNING: Test model file '{TEST_MODEL_PATH}' not found.")
        print("The manager will initialize with random weights for this test.")
        print("Please ensure the model file exists at the specified path to test loading.\n")
        manager = RLManagerBetterCNN(model_path=None) # Test with random weights if not found
    else:
        manager = RLManagerBetterCNN(model_path=TEST_MODEL_PATH)
    ...
    print(f"\nRLManagerBetterCNN Test Complete at (UTC): {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime())}")